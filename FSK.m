...
M1= mhat.*L/2.*exp(-1i*((startphase-2*pi*F1*t_tr)-(2-ctr)));
M1pos=  mhat.*L/2.*exp(1i*((startphase-2*pi*F1*t_tr)+(2-ctr)));

r0pos= (randn(num_bit,1)+randn(num_bit,1)*1i)*sqrt(L/2);
r0neg= (randn(num_bit,1)+randn(num_bit,1)*1i)*sqrt(L/2);
r1pos= M1pos+sqrt(L/2)*(randn(num_bit,1)+randn(num_bit,1)*1i);
...
sig= L/2; %sigma squared
for z= 1:length(SNRdB)
    fun=@(x) (1- igamma(2,x/(2*sig))).*(1/2).*1/sig.*((x./s2(z)).^(1/2)).*exp((-((x./sig)+(s2(z)./sig))/2)).*besseli(1,sqrt((x/sig).*(s2(z)./sig)));
